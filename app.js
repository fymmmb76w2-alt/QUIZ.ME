(() => {
  ...
  function checkAnswer() {
    ...
    if (isCorrect) {
      ...
      // Remove from queue
      state.queue.shift();
      els.nextBtn.textContent = state.queue.length === 0 ? 'Finish' : 'Next';
    } else {
      els.feedback.textContent = 'Incorrect. This question will return later.';
      els.feedback.classList.add('bad');
      // Move to end of queue so it loops back
  ...
  // Keyboard: Enter triggers Check or Next
  document.addEventListener('keydown', e => {
    if (e.key !== 'Enter') return;
    if (!screens.quiz.classList.contains('active')) return;
    if (!els.checkBtn.hidden) checkAnswer();
